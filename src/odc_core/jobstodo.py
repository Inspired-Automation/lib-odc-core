...
#: 0.8.12: the job-level human-assisted-login signal that used to live here
#: (set_human_wait()/set_human_wait_complete()/clear_human_wait(), writing
#: ODC_jobs.human_wait_status/human_wait_deadline/rdp_host/rdp_sessionID)
#: has been removed entirely, not just deprecated. inse-toolkit no longer
#: reads any of those columns - it fully switched to polling Control
#: Room's own job record (assist_phase/assist_url/assist_reason/
#: assist_expires_in_s), fed by a file-based assist.request/assist.release
#: protocol that human_in_loop.request_assist()/release_assist() now write
#: directly into the bot's own per-run job directory. See
#: automation-odc-energia's docs/lib-odc-core-requirements.md item 2 (the
#: explicit ask to stop these writes rather than leave them as
#: harmless-but-dead) and human_in_loop.py's module docstring for the new
#: mechanism. ODC_jobs still has the four now-unused columns - dropping
#: them via DDL is an Outstanding TODO in CLAUDE.md, same pattern as the
#: 0.8.8 rdp_username/rdp_password removal.
#:
#: ODC_suppliers.human_in_loop (surfaced on every get_job_details() row via
#: the LEFT JOIN below) is unrelated to this removed signal - it is still
#: the flag a caller checks to decide whether to call
#: human_in_loop.request_assist()/wait_for_human_login() at all.
